...
Muon optimizer modified from https://github.com/nil0x9/flash-muon
...
###############################
att = [ triton.Config({'BLOCK_SIZE_M': m, 'BLOCK_SIZE_K': k, 'GROUP_SIZE_M': g}, num_stages=s, num_warps=w)
        for m in [32, 64, 128]   for k in [32, 64]   for g in [8]   for s in [3, 4, 5]   for w in [4, 8] ]
@triton.autotune(configs=att, key=['M', 'K'],)
@triton.jit
def mmt_kernel(
        x, y, M, K,
        stride_xm, stride_xk,
