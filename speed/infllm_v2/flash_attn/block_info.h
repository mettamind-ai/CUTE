...
struct BlockInfo {
    ...
    template <typename index_t>
    inline __device__ index_t blockmask_q_offset_flat(const index_t m_block_dim, const int bidb, const int head_idx) const {
        // For flat batch layout (nheads * total_q * dim), we need to include the head index in the offset calculation
        if (sum_s_q == -1) {
            return head_idx * (actual_seqlen_q / m_block_dim) + bidb * (actual_seqlen_q / m_block_dim);
        } else {
            return head_idx * (actual_seqlen_q / m_block_dim) + uint32_t(sum_s_q) / m_block_dim;
        }
    }
