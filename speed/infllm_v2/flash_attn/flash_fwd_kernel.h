...
inline __device__ void compute_block_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    if (Is_causal || Is_local) {
        ...
        // Modified calculation - adjust for group_size:
        const int orig_seqlen_q = binfo.actual_seqlen_q / params.m_block_dim;
        n_block_max = std::min(n_block_max,
                            cute::ceil_div(((m_block + 1) * kBlockM) / params.m_block_dim + binfo.actual_seqlen_k - orig_seqlen_q + params.window_size_right, kBlockN));
        
        // Alternative approach (more readable):
        // Map to original sequence coordinates first then back to blocks
        // const int query_pos_expanded = (m_block + 1) * kBlockM;  // Last position in this block in expanded space
        // const int query_pos_original = query_pos_expanded / group_size;  // Corresponding position in original space
        // n_block_max = std::min(n_block_max,
        //                       cute::ceil_div(query_pos_original + binfo.actual_seqlen_k - (binfo.actual_seqlen_q / group_size) + params.window_size_right, kBlockN));
    }
    ...
    empty_line_flag = empty_line_flag || max_no_larger_idx == -1 || max_no_larger_idx < n_block_min;
    ...
    int mask_val = blockmask.max_no_larger(n_block);
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if(is_skip){
            // if(threadIdx.x == 0) {
            //     printf("fwd skip masking_step=%d, n_block=%d, next_block_col_idx=%d\n", masking_step, n_block, next_block_col_idx);
            // }
            ...
            if(!is_last_block){ //is_skip==false
                mask_val = blockmask.max_no_larger(n_block - 1);
                is_last_block = mask_val == -1;
    ...
    // These are the iterations where we don't need masking on S
    leap = n_block - next_block_col_idx + 1;
    ...
    // return;
    for(n_block = next_block_col_idx; n_block != -1 && n_block >= n_block_min; n_block = next_block_col_idx){
        // if(bidb == 0 && bidh == 0 && m_block == 0 && threadIdx.x == 0){
        ...
        // if(threadIdx.x == 0 && n_block - 1 != next_block_col_idx) {
        //     printf("fwd skip second loop n_block=%d, next_block_col_idx=%d\n", n_block, next_block_col_idx);
        // }
...
inline __device__ void compute_block_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    // Initialize blockmask skip logic
    fwdIterator</*Is_streaming=*/false, /*Is_exact_streaming=*/false, /*Is_batched=*/true> blockmask(params, binfo, kBlockM, kBlockN, bidb, bidh, m_block, n_block_min, n_block_max);
    int next_block_col_idx = blockmask.max_no_larger(n_block_max-1);
    bool is_last_block = next_block_col_idx == -1;
    int mask_val = -1;
    int leap = 0;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if(is_skip){
            ...
            leap = is_last_block ? 0 : leap;
            // if(threadIdx.x == 0){
            //     printf("Skipping masking: seq_q=%d, head=%d, m_block=%d, n_block=%d, next_block_col_idx=%d, leap=%d, is_last_block=%d\n", 
            //        binfo.actual_seqlen_q, bidh, m_block, n_block, next_block_col_idx, leap, is_last_block);
            ...
            if (masking_step > 0) {
                // if(threadIdx.x == 0){
                //     printf("Advancing gV: seq_q=%d, head=%d, m_block=%d, n_block=%d, next_block_col_idx=%d, leap=%d, is_last_block=%d\n", 
                //        binfo.actual_seqlen_q, bidh, m_block, n_block, next_block_col_idx, leap, is_last_block);
                // }
                tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
                flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tVgV, tVsV, tKVcKV, tKVpKV);
            ...
            cute::cp_async_fence();
            // if(threadIdx.x == 0){
            //     printf("Processing masking: seq_q=%d, head=%d, m_block=%d, n_block=%d, next_block_col_idx=%d, leap=%d, is_last_block=%d\n", 
            //        binfo.actual_seqlen_q, bidh, m_block, n_block, next_block_col_idx, leap, is_last_block);
    ...
    for(n_block = next_block_col_idx; n_block != -1 && n_block >= n_block_min; n_block = next_block_col_idx){
        ...
        leap = n_block - next_block_col_idx;
        // if(threadIdx.x == 0){
        //     printf("Processing non-masking: seq_q=%d, head=%d, m_block=%d, n_block=%d mask_val=%d, next_block_col_idx=%d, leap=%d, is_last_block=%d\n", 
        //        binfo.actual_seqlen_q, bidh, m_block, n_block, mask_val, next_block_col_idx, leap, is_last_block);
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    static_assert(kNThreads == 32, "We assume that each block has 32 threads");
...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Split, bool Append_KV, typename Params>
inline __device__ void compute_block_attn_splitkv(const Params &params) {
    const int m_block = blockIdx.x;
    // The block index for the batch.
    const int bidb = Split ? blockIdx.z / params.h : blockIdx.y;
    // The block index for the head.
    ...
    const int head_mask_type = params.head_mask_type[bidh];
    flash::compute_block_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
