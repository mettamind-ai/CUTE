...
parser.add_argument("--ohmai", type=int, default=0)         # ... để dùng hết cache line mỗi lần triton đọc dữ liệu
...
if  args.L: # (L)arge ~ 999m
    ...
    model = WinGPT(dim=1664, n_layers=20, num_heads=16, num_kv_heads=4, head_dim=128,
        vocab_size=args.vocab, max_seq_len=tokens_per_batch, active_vocab=args.ohmai,)
else:       # (S)mall ~ 333m
