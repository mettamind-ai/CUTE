...
__global__ void kernel_forward( const int B, const int T, const int C,
    ...
    // Mỗi cuda thread sẽ xử lý dữ liệu theo chiều sequence T
    // Xác định index của thread hiện tại
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;

    // Vì idx = _b * C + _c đại diện cho phần tử (_b, _c) của ma trận (B, C)
    // nên từ idx ta có thể tính ngược ra _b và _c
    const int _b = idx / C; // hàng _b: sample sequence đang xét (các tokens liên tiếp chiều dài ctx_length)
    const int _c = idx % C; // cột _c: channel đang xét

    // Sample _b chứa (T * C) phần tử kiểu số thực bf16
    const int _offset = _b * T * C + _c; // offset để trỏ tới các scalar values của channel đang xét

    float u = _u[_c]; // u của channel đang xét, là giá trị cho trước
    float w = _w[_c]; // w của channel đang xét, là giá trị cho trước
    ...
    for (int i = 0; i < T; i++) {
        const int ii = i * C;
        const float kk = float( k[ii] ); // chuyển bf16 thành float32
        const float vv = float( v[ii] ); // chuyển bf16 thành float32

        // Tính toán trên float32 để không bị overflow
        float ww = u + kk; // ww là u và k của channel đang xét
        float p = max(pp, ww);
        float e1 = exp(pp - p);
        float e2 = exp(ww - p);

        // Chuyển hóa float32 ngược về bf16 rồi ghi kết quả ra y
