...
"""
    ...
    from trainer7 import train_callback, generate_init_weight
    from dataset import MyDataset

    train_data = MyDataset(args)
    args.vocab_size = train_data.vocab_size

    from racoon7 import Racoon as RWKV
    model = RWKV(args)

    if len(args.load_model) == 0 or args.train_stage == 1:  # shall we build the initial weights?
        init_weight_name = f"{args.proj_dir}/rwkv-init.pth"
        generate_init_weight(model.rwkv, init_weight_name)  # save initial weights
