...
RUN_NAME="Prolong64k-LongCE_L"$N_LAYER"-D"$N_EMBD"-C"$CTX_LEN"_ML"$N_MOBA_LAYER"_CS"$MOBA_CHUNK_SIZE"_TK"$MOBA_TOPK
PROJ_DIR="out/$RUN_NAME"
#
MODEL_FILE="out/Prolong64k-20B_L32-D2560-C64000_ML8_CS2048_TK3/rwkv-0.pth"
...
#
M_BSZ="2" # takes ~9G VRAM here => reduce this to save VRAM, increase this for faster speed
LR_INIT="1e-5"
LR_FINAL="1e-5"
...
python train.py --load_model $MODEL_FILE --wandb $WANDB --run_name $RUN_NAME --proj_dir $PROJ_DIR \
 --ctx_len $CTX_LEN --my_pile_stage 3 --epoch_count 1 --epoch_begin 1 --epoch_steps 1000 \
 ...
 --accelerator gpu --devices $GPU_PER_NODE --precision bf16 --strategy deepspeed_stage_2 --grad_cp $GRAD_CP \
 --enable_progress_bar True --ds_bucket_mb $DS_BUCKET_MB \
 --n_moba_layer $N_MOBA_LAYER --moba_chunk_size $MOBA_CHUNK_SIZE --moba_topk $MOBA_TOPK --use_longce