"""Low-Rank Adaptation (LoRA) kernels. Modded from github.com/axolotl-ai-cloud/axolotl
"""
...
def get_lora_parameters(proj:nn.Module):
    """Returns: A tuple containing the base weight matrix, LoRA A matrix, LoRA B matrix, and scaling factor."""
    W = base_layer.weight
    adapter = proj.active_adapters[0] if hasattr(proj, "active_adapters") else proj.active_adapter
    quant = getattr(W, "quant_state", None)

    A = proj.lora_A[ adapter].weight
    B = proj.lora_B[ adapter].weight
    s = proj.scaling[adapter]
    return W, quant, A, B, s


def matmul_lora( X_TC:Tensor, W:Tensor, W_quant, A:Tensor, B:Tensor, s:float,out:Tensor|None=None,) -> Tensor:
    """Efficient fused matmul + LoRA computation. Return X @ W + X @ A @ B"""
    dtype = X_TC.dtype
    # W = dequantize(W.t(), W_quant)
    out = torch.matmul(X_TC, W, out=out)
