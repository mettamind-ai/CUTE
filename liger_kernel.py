...
# https://github.com/linkedin/Liger-Kernel/blob/main/src/liger_kernel/utils.py
# https://github.com/linkedin/Liger-Kernel/blob/main/src/liger_kernel/ops/utils.py
def infer_device():
    """ Get current device name based on available devices """
    if torch.cuda.is_available(): return "cuda"  # Works for both Nvidia and AMD
    elif torch.xpu.is_available(): return "xpu"
    else: return "cpu"

amp_custom_fwd = functools.partial(torch.amp.custom_fwd, device_type=infer_device())
amp_custom_bwd = functools.partial(torch.amp.custom_bwd, device_type=infer_device())
...
def embedding_backward_kernel(
    ...
    weight_locs = grad_weight_ptr + grad_weight_offsets
    grad_weight = tl.load(weight_locs, mask=mask_m[:, None] & mask_n[None, :])
    tl.store(weight_locs, grad_output + grad_weight,  mask=mask_m[:, None] & mask_n[None, :])
