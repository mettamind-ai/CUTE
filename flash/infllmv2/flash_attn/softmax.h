...
inline __device__ void apply_mask_local(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
    ...
    for (int mi = 0; mi < size<0, 1>(tensor); ++mi) {
        ...
        for (int i = 0; i < size<0, 0>(tensor); ++i) {
            ...
            // Map expanded position back to original position
...
inline __device__ void apply_mask_streaming(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
                                        const int max_seqlen_k, const int row_idx_offset,
                                        const int max_seqlen_q, const int warp_row_stride,
                                        const int local_size, const int sink_size, const int m_block_dim=1) {
    // tensor has shape (ncol=(2, MMA_M), nrow=(2, MMA_N))
    static_assert(Layout::rank == 2, "Only support 2D Tensor");
    const int lane_id = threadIdx.x % 32;
    const int col_idx_offset = col_idx_offset_ + (lane_id % 4) * 2;
    ...
    for (int mi = 0; mi < size<0, 1>(tensor); ++mi) {
        ...
        for (int i = 0; i < size<0, 0>(tensor); ++i) {
            ...
            // Calculate limits based on original positions
            const int col_idx_limit_left = std::max(0, orig_row_idx + max_seqlen_k - orig_max_seqlen_q - (local_size-1));
            const int col_idx_limit_right = std::min(max_seqlen_k, orig_row_idx + 1 + max_seqlen_k - orig_max_seqlen_q);
