...
__global__ void flash_bwd_block_dq_dk_dv_loop_seqk_parallel_kernel(Flash_bwd_params params) {
    static_assert(!(Is_causal && Is_local));  // If Is_local is true, Is_causal should be false
    flash::compute_block_dq_dk_dv_seqk_parallel<Kernel_traits, Is_dropout, Is_causal, Is_local, Is_even_MN, Is_even_K>(params);
}
...
// for blocksparse-flash-attention2
template<typename Kernel_traits, bool Is_dropout>
void run_flash_bwd_block_seqk_parallel(Flash_bwd_params &params, cudaStream_t stream, const bool configure) {
    const int num_m_block = (params.seqlen_q + Kernel_traits::kBlockM - 1) / Kernel_traits::kBlockM;
    dim3 grid_m(num_m_block, params.b, params.h);
    const int num_n_block = (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN;
    int gridDimx = num_n_block;
...
// for blocksparse-flash-attention2
template<typename Kernel_traits, bool Is_dropout>
void run_flash_bwd_block(Flash_bwd_params &params, cudaStream_t stream, const bool configure) {
    if (configure) return;
    run_flash_bwd_block_seqk_parallel<Kernel_traits, Is_dropout>(params, stream, configure);
}
...
void run_mha_bwd_hdim64(Flash_bwd_params &params, cudaStream_t stream, const bool configure) {
    ...
    BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        ...
        if (max_smem_per_block >= 144 * 1024) {
            run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 16, 64, 4, 1, 4, 1, false, false, T>, Is_dropout>(params, stream, configure);
            ...
            // if (params.h == params.h_k) {
                ...
                run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 16, 64, 4, 1, 4, 1, true, false, T>, Is_dropout>(params, stream, configure);
                // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, false, false, T>, Is_dropout>(params, stream, configure);
...
template<typename T>
void run_mha_bwd_block_hdim64(Flash_bwd_params &params, cudaStream_t stream, const bool configure) {
    constexpr static int Headdim = 64;
    int device;
    cudaGetDevice(&device);
    int max_smem_per_block;
    ...
    BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        ...
        // This is slightly faster. We want to split M more so we need fewer registers to store LSE.
        if (max_smem_per_block >= 144 * 1024) {
            run_flash_bwd_block<Flash_bwd_kernel_traits<Headdim, 16, 64, 4, 1, 4, 1, false, true, T>, Is_dropout>(params, stream, configure);
            // This has a lot of register spilling
            // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 128, 8, 4, 4, 4, true, false, T>, Is_dropout>(params, stream, configure);
        } else {
            // if (params.h == params.h_k) {
                // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 64, 128, 8, 2, 4, 4, false, false, T>, Is_dropout>(params, stream, configure);
                run_flash_bwd_block<Flash_bwd_kernel_traits<Headdim, 16, 64, 4, 1, 4, 1, true, true, T>, Is_dropout>(params, stream, configure);
                // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, false, false, T>, Is_dropout>(params, stream, configure);
                // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, true, false, T>, Is_dropout>(params, stream, configure);
            // } else {
            //     run_flash_bwd_seqq_parallel<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, false, false, T>, Is_dropout>(params, stream, configure);
...
template<typename T>
void run_mha_bwd_block_hdim128(Flash_bwd_params &params, cudaStream_t stream, const bool configure) {
    constexpr static int Headdim = 128;
    int device;
    cudaGetDevice(&device);
    int max_smem_per_block;
