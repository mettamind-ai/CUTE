...
def test_causal_conv1d_update_with_batch_gather(dim, width, seqlen, has_cache_seqlens, has_bias, silu_activation, itype):
    ...
    weight = torch.randn(dim, width, device=device, dtype=torch.float32, requires_grad=True)
    if has_bias: bias = torch.randn(dim, device=device, dtype=torch.float32, requires_grad=True)
    else: bias = None
...
@pytest.mark.parametrize("itype", [torch.float32, torch.bfloat16])
# @pytest.mark.parametrize('itype', [torch.float16])
@pytest.mark.parametrize("dim", [2048, 2048 + 16])
...
def test_causal_conv1d_varlen(dim, seqlen, width, has_bias, silu_activation, itype):
    ...
    seq_idx = torch.stack([torch.cat([torch.full((s,), i, dtype=torch.int32, device=device) for i, s in enumerate(sl)], dim=0) for sl in seqlens], dim=0)
    x_ref = x.detach().clone().requires_grad_()
    weight_ref = weight.detach().clone().requires_grad_()
    bias_ref = bias.detach().clone().requires_grad_() if bias is not None else None
    activation = None if not silu_activation else "silu"
    ...
    print(f"dx max diff: {(x.grad - x_ref.grad).abs().max().item()}")
    print(f"dweight max diff: {(weight.grad - weight_ref.grad).abs().max().item()}")
    if has_bias: print(f"dbias max diff: {(bias.grad - bias_ref.grad).abs().max().item()}")
...
if __name__ == "__main__":
    test_causal_conv1d(dim=64, seqlen=128, width=3, has_bias=True, silu_activation=False, itype=torch.float32, channel_last=False, has_initial_states=False, return_final_states=False)
