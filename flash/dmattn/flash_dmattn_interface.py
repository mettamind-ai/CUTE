...
print(f"flash_dmattn_0.1.0: free_memory_gb {free_memory_gb}, max_jobs {os.environ['MAX_JOBS']}")
...
flash_dmattn_cuda = torch.utils.cpp_extension.load(
    "CUTE_flash_dmattn_cuda",
    sources=[
        abspath/"flash_api.cpp", 
        abspath/"flash_fwd_hdim128_bf16_causal_sm80.cu", 
        abspath/"flash_fwd_hdim128_bf16_sm80.cu", 
        abspath/"flash_fwd_split_hdim128_bf16_causal_sm80.cu", 
        abspath/"flash_fwd_split_hdim128_bf16_sm80.cu", 
    ],
    extra_cuda_cflags=NVCC_FLAGS,
    extra_include_paths=[ str(abspath), str(abspath/"../cutlass/include"),],
)
# ~/.cache/torch_extensions/...
print(f"flash_dmattn_0.1.0: DONE. In {int(time.time() - started_at)} seconds.")
#########################################################################
...
flash_dmattn_gpu = flash_dmattn_cuda # type: ignore
...
class FlashDMAttnVarlenFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if is_causal is None: is_causal = False
        if softcap is None: softcap = 0.0
        if deterministic is None: deterministic = True
        if return_softmax is None: return_softmax = False
