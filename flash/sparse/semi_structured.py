...
mySparseSemiStructuredLinear_cuda = torch.utils.cpp_extension.load(
    "CUTE_mySparseSemiStructuredLinear_cuda",
    sources=[abspath/"mySparseSemiStructuredLinear_cuda.cu"],
    extra_cuda_cflags=NVCC_FLAGS,
    extra_include_paths=[ str(abspath/"../cutlass/include") ],
)

def sparse_semi_structured_linear(input, weight, meta, bias_opt=None, activation_opt=None):
    return mySparseSemiStructuredLinear_cuda.\
        my_sparse_semi_structured_linear(input, weight, meta, bias_opt, activation_opt)
...
_SEMI_STRUCTURED_SPARSE_CONFIG = namedtuple(
    ...
    "compression_factor sparse_min_rows sparse_min_cols dense_min_rows dense_min_cols"
)
