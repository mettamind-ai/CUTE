...
class AttentionMask:
    ''' Apply 2 loại mask:
    1/ Sequence length mask: Che các vị trí vượt quá độ dài thực
    2/ Causal mask: Che các vị trí "tương lai"
    '''
    def __init__(
        self,
    ...
    def apply_mask(
        ...
        # We use t0ScS as these indices are known at compile time.
        # We then must subtract the column limit by the thread column offset.
        ...
        if not mask_causal:
            ...
            if cutlass.const_expr(self.qhead_per_kvhead_packgqa > 1):
                ...
                mma_m_idx = (m_block * self.m_block_size + tScS_mn[tidx % threads_per_row, 0][0]) \
            ...
            for r in range(cute.size(tScS_mn.shape[0])):
                ...
                else:   row_idx = utils.shuffle_sync(mma_m_idx, r % threads_per_row, width=threads_per_row)
                ...
                for c in range(cute.size(tScS_mn.shape[1])):
                    ...
                    if t0ScS_mn[0, c][1] >= col_limit_right: acc_S_mn[r, c] = -cutlass.Float32.inf
