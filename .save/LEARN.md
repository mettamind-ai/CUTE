...
## Gemma3n: Flow đơn giản hóa KHÔNG có ALTUP
```python
# Input của layer
hidden_states    # [batch, seq_len, hidden_size]
per_layer_input  # [batch, seq_len, hidden_size_per_layer_input]

# Bước 1: Xử lý bình thường qua layer
hidden_states = LayerNorm(hidden_states)
hidden_states = Attention(hidden_states) + hidden_states
hidden_states = LayerNorm(hidden_states)  
hidden_states = MLP(hidden_states)       + hidden_states

# Bước 2: Tạo gate từ hidden_states và Apply gate lên per_layer_input
gate          = Linear(hidden_states)       # [batch, seq_len, hidden_size_per_layer_input]
gated_input   = activation(gate) * per_layer_input

# Bước 3: Project về hidden_size và cộng vào hidden_states
gated_input   = Linear(gated_input)         # → [batch, seq_len, hidden_size]
