...
def create_block(
    ...
    # Mixer
    if   arch in "t|T": mixer_cls = partial(CausalMHA,    **attn_cfg, **factory_kwargs, layer_idx=layer_idx)
    elif arch in "m|M": mixer_cls = partial(Mamba2Wrapper, **ssm_cfg, **factory_kwargs, layer_idx=layer_idx)
    else: raise NotImplementedError
    # MLP
    if   arch in "T|M": mlp_cls = partial(SwiGLU, d_intermediate=d_intermediate, **factory_kwargs)
