...
def load_balancing_loss(
    ...
    NOTE: This is the loss we used for all experiments. It computes the loss on each device/minibatch, 
    and then averages the loss over all devices/minibatches. It is possible that computing the loss on each
    example is better, or that computing the statistics over the entire (global) batch would have been better.

    Args:
        router_output: The output of the routing module.
        N: The number of "experts", i.e. the downsampling factor. Can be a float (e.g. 2.5) 
        or an integer (e.g. 3), but must be greater than 1.
